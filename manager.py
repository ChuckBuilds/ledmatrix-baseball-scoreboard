...
class BaseballScoreboardPlugin(BasePlugin):
    ...
    def display(self, display_mode: str = None, force_clear: bool = False) -> None:
        ...
        Replicates display logic from SportsLive, SportsRecent, and SportsUpcoming.
        ...
        if not display_mode:
            # Auto-select mode based on available games and live_priority settings
            display_mode = self._select_display_mode()

        # If no display mode selected (no games in any mode), just return
        # This matches old manager behavior - don't update display if no content
        if not display_mode:
            return
        ...
        # Render the game
        mode_suffix = display_mode.replace('baseball_', '')
    ...
    def _select_display_mode(self) -> str:
        """
        Select display mode based on available games and live_priority settings.

        Returns:
            Display mode string
        """
        # Check for live games with live_priority enabled
        for league_key, league_config in self.leagues.items():
            if not league_config.get('enabled', False):
                continue
        ...
        for league_key, league_config in self.leagues.items():
            ...
            recent_state = self.league_state[league_key]['recent']
            if recent_state['games_list']:
                return 'baseball_recent'
        ...
        for league_key, league_config in self.leagues.items():
            ...
            upcoming_state = self.league_state[league_key]['upcoming']
            if upcoming_state['games_list']:
                return 'baseball_upcoming'

        # No games in any mode - return None to signal no content
        # This matches old manager behavior of just returning without updating display
        return None
    ...
    def _get_current_game_for_mode(self, display_mode: str, current_time: float):
        ...
        # Collect all games from enabled leagues for this mode
        all_games = []
        for league_key, league_config in self.leagues.items():
            if not league_config.get('enabled', False):
                continue
            ...
            # Add current game to selection pool
            if state['current_game']:
                current_game = state['current_game'].copy()
                current_game['_league_key'] = league_key
                current_game['_priority'] = league_config.get('live_priority', False) if mode_suffix == 'live' else False
                all_games.append(current_game)

        if not all_games:
            return None, None

        # Sort: prioritize live_priority, then favorites
        def sort_key(game):
            priority = game.get('_priority', False)
            is_favorite = self.game_filter.is_favorite_game(game, game.get('league_config', {}).get('favorite_teams', []))
            return (not priority, not is_favorite)

        all_games.sort(key=sort_key)
        selected_game = all_games[0]
        league_key = selected_game.pop('_league_key', None)
        selected_game.pop('_priority', None)

        return selected_game, league_key
